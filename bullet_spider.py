...
class OpportunityCategory(str, Enum):
    ...
    STICKER = "sticker"
    LOGO = "logo"
    CHILDREN_COLORING_BOOK = "children_coloring_book"
...
class DemandSignal:
    ...
    category_scorecard: Mapping[str, Any] = field(default_factory=dict)
    profit_evidence_gate: Mapping[str, Any] = field(default_factory=dict)
...
BULLET_CONTRACT_VERSION = "bullet_drafts.v1"
MARKET_SPY_CONTRACT_VERSION = "market_spy_handoff.v1"


class MarketSpyHandoffAdapter:
    """Adapter for versioned Market Spy handoff list envelopes.

    It preserves action-driving evidence and rejects legacy/unversioned envelopes,
    rather than filling absent evidence with permissive defaults.
    """
    name = "market_spy_handoff"

    ROUTES = (
        "affiliate_spider", "digital_product_spider", "ebook_spider",
        "design_asset_spider", "wall_art_decor_spider", "sticker_spider",
        "logo_spider", "children_coloring_book_spider",
        "illustrated_story_spider", "physical_product_spider",
        "category_candidates_requiring_data", "affiliate_candidates_requiring_data",
    )

    def __init__(self, envelope: Mapping[str, Any]):
        handoff = envelope.get("handoff", envelope)
        if handoff.get("schema_version") != MARKET_SPY_CONTRACT_VERSION:
            raise ValueError("Unsupported or missing Market Spy handoff schema_version")
        if handoff.get("execution_allowed") is not False:
            raise ValueError("Market Spy handoff must explicitly block execution")
    ...
    def collect(self, request: Mapping[str, Any]) -> Sequence[DemandSignal]:
        ...
        for route in self.ROUTES:
            payload = self._handoff.get(route, [])
            if not isinstance(payload, list):
                raise ValueError(f"Market Spy handoff route {route} must be a list")
            for row in payload:
                identity = str(row.get("opportunity_id") or row.get("product") or "") + "|" + str(row.get("observed_at") or "")
                if identity not in seen:
                    rows.append(row)
                    seen.add(identity)
        return [signal_from_mapping(row, strict_market_spy=True) for row in rows]
...
def signal_from_mapping(row: Mapping[str, Any], strict_market_spy: bool = False) -> DemandSignal:
    ...
    metric_name = row.get("metric_name") or row.get("metric_or_proxy") or row.get("metric")
    if not metric_name and strict_market_spy:
        raise ValueError("Market Spy handoff is missing metric_or_proxy")
    metric_name = metric_name or "unspecified_proxy"
    ...
    if metric_kind not in {"proxy", "verified_sales", "first_party_aggregate"}:
        metric_kind = "proxy"
    category_scorecard = row.get("category_scorecard") or {}
    profit_gate = row.get("profit_evidence_gate") or {}
    evidence = profit_gate.get("evidence") or {}
    components = category_scorecard.get("components") or {}
    if strict_market_spy and (not category_scorecard or not profit_gate):
        raise ValueError("Market Spy handoff must preserve category_scorecard and profit_evidence_gate")
    def required_score(name: str, *fallbacks: str, invert: bool = False) -> float:
        candidates = (name,) + fallbacks
        value = next((components.get(k) for k in candidates if components.get(k) is not None), None)
        if value is None:
            value = next((evidence.get(k) for k in candidates if evidence.get(k) is not None), None)
        if value is None:
            if strict_market_spy:
                return 0.0
            return 0.5
        value = float(value) / 100.0 if float(value) > 1 else float(value)
        return 1.0 - value if invert else value
    gate_no_go = str(profit_gate.get("decision", "")).replace("-", "_") == "no_go"
    return DemandSignal(
        ...
        metric_value=float(row.get("metric_value", row.get("value", row.get("opportunity_score", 0.0)))),
        metric_kind=metric_kind,
        previous_value=(None if row.get("previous_value") is None else float(row["previous_value"])),
        confidence=_bounded(row.get("confidence", required_score("confidence"))),
        competition=_bounded(row.get("competition", row.get("competition_score", required_score("competition", invert=True)))),
        supply_gap=_bounded(row.get("supply_gap", row.get("supply_gap_score", 0.0 if strict_market_spy else 0.5))),
        producibility=_bounded(row.get("producibility", 0.0 if strict_market_spy else 0.5)),
        price_margin=_bounded(row.get("price_margin", row.get("margin_score", required_score("margin_potential")))),
        price=(None if row.get("price", evidence.get("price")) is None else float(row.get("price", evidence.get("price")))),
        currency=row.get("currency"),
        commission_rate=(None if row.get("commission_rate") is None else float(row["commission_rate"])),
        production_cost=(None if row.get("production_cost", evidence.get("production_cost")) is None else float(row.get("production_cost", evidence.get("production_cost")))),
        production_time_hours=(None if row.get("production_time_hours", evidence.get("production_time")) is None else float(row.get("production_time_hours", evidence.get("production_time")))),
        risk=_bounded(row.get("risk", row.get("risk_score", 1.0 if gate_no_go else (0.5 if not strict_market_spy else 0.0)))),
        language=row.get("language"),
        category=str(row.get("category") or row.get("market_category") or OpportunityCategory.DIGITAL_PRODUCT.value),
...
def _category(value: str) -> OpportunityCategory:
    aliases = {
        ...
        "design_asset": OpportunityCategory.PRODUCT_DESIGN,
        "wall_art_decor": OpportunityCategory.DECOR_ART,
        "ebook": OpportunityCategory.DIGITAL_BOOK,
...
def _category_specific_blockers(signal: DemandSignal) -> tuple[str, ...]:
    category = _category(signal.category)
    components = signal.category_scorecard.get("components", {})
    required = {
        OpportunityCategory.PRODUCT_DESIGN: ("rights_risk",),
        OpportunityCategory.DECOR_ART: ("rights_risk", "fulfillment_risk"),
        OpportunityCategory.STICKER: ("format_printability", "licensing_risk"),
        OpportunityCategory.LOGO: ("originality_review", "similarity_review", "trademark_ip_risk"),
        OpportunityCategory.CHILDREN_COLORING_BOOK: ("age_band", "format_printability", "print_economics", "child_safety", "illustration_rights_risk"),
        OpportunityCategory.ILLUSTRATED_STORY: ("age_band", "format_printability", "illustration_rights_risk", "user_supplied_topic", "user_supplied_message"),
        OpportunityCategory.PHYSICAL_PRODUCT: ("fulfillment_risk",),
    }.get(category, ())
    blockers = [f"missing category check: {key}" for key in required if components.get(key) is None] if signal.category_scorecard else []
    if signal.profit_evidence_gate:
        decision = str(signal.profit_evidence_gate.get("decision", "")).replace("-", "_")
        if decision != "go": blockers.append("Market Spy profit evidence gate is no_go")
    if signal.category_scorecard and not signal.category_scorecard.get("eligible", False):
        blockers.append("Market Spy category scorecard is ineligible")
...
def _production_gate(signal: DemandSignal, intent: float, urgency: float, as_of: datetime) -> tuple[str, tuple[str, ...]]:
    ...
    blockers.extend(_category_specific_blockers(signal))
...
def assess_signal(signal: DemandSignal, as_of: datetime | None = None) -> OpportunityAssessment:
    ...
    return OpportunityAssessment(
        ...
        category_scorecard=signal.category_scorecard,
        profit_evidence_gate=signal.profit_evidence_gate,
    )
...
def build_production_brief(assessment: OpportunityAssessment, language: str | None = None) -> ProductionBrief:
    ...
    category_map = {
        ...
        OpportunityCategory.DECOR_ART: ("wall_art_decor_spider", "visual_spider", "qa_spider"),
        OpportunityCategory.STICKER: ("sticker_spider", "visual_spider", "qa_spider"),
        OpportunityCategory.LOGO: ("logo_spider", "visual_spider", "qa_spider"),
        OpportunityCategory.CHILDREN_COLORING_BOOK: ("children_coloring_book_spider", "book_spider", "qa_spider"),
        OpportunityCategory.ILLUSTRATED_STORY: ("illustrated_story_spider", "book_spider", "visual_spider", "qa_spider"),
        OpportunityCategory.PHYSICAL_PRODUCT: ("physical_product_spider", "qa_spider"),
    }
    spider_map = {
    ...
    return ProductionBrief(
        ...
        review_gates=REVIEW_GATES + tuple(f"category_check:{key}" for key in assessment.category_scorecard.get("components", {})),
...
def run_bullet_spider(
    ...
    if not adapters:
        return {
            "schema_version": BULLET_CONTRACT_VERSION,
    ...
    return {
        ...
        "limitations": [
            ...
            "Allowed automatically: public monitoring, scoring, rejection, briefs, and listing drafts.",
            "Forbidden: publishing, spending, account creation, or messaging as the user.",
